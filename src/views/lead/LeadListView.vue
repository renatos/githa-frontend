<template>
  <div class="lead-list-view p-4 md:p-6 flex flex-col gap-6">
    ...
    >
      ...
      <!-- Custom Cell: Opt Out -->
      <template #cell-optOut="{ value }">
        <span
          v-if="value"
          class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300"
          title="Lead solicitou Não Perturbe"
        >
          <i class="fa-solid fa-ban"></i> Opt-Out
        </span>
        <span
          v-else
          class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300"
        >
          Ativo
        </span>
      </template>
...
const columns = [
  ...
  { key: 'phone', label: 'Telefone', sortable: true, width: '140px' },
  ...
  { key: 'optOut', label: 'Opt-Out', align: 'center', width: '100px', sortable: true },
